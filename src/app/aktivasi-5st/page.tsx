...
import { Upload, Check, Loader2, Zap, X, Lock, BadgeCheck, Download } from 'lucide-react';
import { saveQris } from '@/lib/saveQris';

const PRICE = 'Rp 85.000';
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/v1/activation/request`, {
        ...
        body: JSON.stringify({ app: 'stc', feature: 'blitz5s', name: name.trim(), stockityId: sid.trim(), proof }),
      });
        ...
        <div style={sx.wrap}>
          <div style={{ ...sx.card, textAlign: 'center', padding: '44px 26px' }}>
            ...
            <p style={{ fontSize: 14.5, lineHeight: 1.6, color: '#9aa6a1', maxWidth: 360, margin: '0 auto' }}>
              Pengajuan kamu <b style={{ color: '#f4f6f5' }}>sedang diproses</b>, mohon menunggu. Pembayaran diverifikasi admin dan Mode <b style={{ color: '#f4f6f5' }}>5st</b> akan aktif pada akun <b style={{ color: AC }}>{sid}</b> biasanya dalam ~10 menit.
            </p>
            <div style={{ marginTop: 18, padding: '13px 15px', borderRadius: 14, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.22)', maxWidth: 360, marginInline: 'auto' }}>
              ...
              <a href="mailto:supportstockity@gmail.com?subject=Aktivasi%205st%20belum%20diproses" style={{ display: 'inline-block', marginTop: 8, fontSize: 13, fontWeight: 700, color: AC, textDecoration: 'none' }}>✉️ supportstockity@gmail.com</a>
            </div>
          </div>
        </div>
      </div>
    ...
    <div style={sx.page}>
      <div style={sx.wrap}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 26 }}>
          <div style={sx.heroIcon}><Zap style={{ width: 28, height: 28, color: '#04210b' }} /></div>
          <h1 style={{ fontSize: 26, fontWeight: 700, letterSpacing: '-0.6px', lineHeight: 1.12, color: '#f4f6f5' }}>Aktivasi Mode 5st</h1>
          <p style={{ fontSize: 14.5, color: '#9aa6a1', marginTop: 8, lineHeight: 1.55, maxWidth: 380, marginInline: 'auto' }}>
            Buka mode 5st — order blitz keluar hasil 5 detik — di STC AutoTrade. Langganan <b style={{ color: AC }}>{PRICE} / bulan</b>.
          </p>
        ...
        <div style={sx.note}>
          ...
          <p style={{ fontSize: 13, lineHeight: 1.55, color: '#c4cec9' }}>
            Setelah pembayaran diverifikasi, mode <b style={{ color: '#f4f6f5' }}>5st</b> otomatis terbuka di akun kamu — aktifkan toggle 5st di panel Fastrade FTT. Langganan berlaku per bulan.
          </p>
        </div>
