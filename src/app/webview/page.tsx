...
export default function PanduanPage() {
  ...
  const cell = (v: Cell) =>
    v === 'ok'
      ? <span style={{ display: 'inline-flex', width: 22, height: 22, borderRadius: 7, background: 'rgba(16,185,129,0.14)', alignItems: 'center', justifyContent: 'center' }}><Check size={14} strokeWidth={2.6} color="#10b981" /></span>
      : v === 'lim'
        ...
        <div style={S.kompKartu}>
          <div style={S.kompKepala}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Layers size={16} strokeWidth={2} color="var(--accent, #10b981)" />
...
const S: Record<string, React.CSSProperties> = {
  ...
  cahaya: { position: 'absolute', top: -140, left: '50%', transform: 'translateX(-50%)', width: 440, height: 300, borderRadius: '50%', background: 'var(--accent, #10b981)', opacity: 0.08, filter: 'blur(90px)', pointerEvents: 'none' },
  wadah: { position: 'relative', maxWidth: 640, margin: '0 auto', padding: '26px 16px 44px' },

  kepala: { marginBottom: 22, textAlign: 'center' },
  lencana: { display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 99, fontSize: 11.5, fontWeight: 700, color: 'var(--accent, #10b981)', background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.25)', marginBottom: 14 },
  ...
  tautan: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 12, padding: '14px 18px', borderRadius: 15, fontSize: 14.5, fontWeight: 700, color: '#04210b', background: 'var(--accent, #10b981)', border: 'none', textDecoration: 'none' },
